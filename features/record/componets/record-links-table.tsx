...
import { useRecordLinks } from "../hooks/use-record";
import { useRecordLinksTableState } from "../hooks/use-table-state";
import { RecordLink } from "../types";

const columns: ColumnDef<RecordLink>[] = [
  {
    accessorKey: "id",
    header: "Link",
    cell: ({ row }) => {
      const link = `${BASE_URL}/student/verify-link/${row.original.id}`;
      return link;
    },
  },
        ...
        <>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <Link href={`/admin/records/${recordId}/links/${id}`}>
...
export function RecordLinksTable({ recordId }: { recordId: string }) {
  const tableState = useRecordLinksTableState();

  const { data, isPending, isFetching, status, refetch } = useRecordLinks(
    recordId,
    {
            ...
            {isPending ? (
              ...
              <TableRow>
                ...
                >
                  ...
                  {status === "success" && <div>No links found.</div>}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
...
function Toolbar(props: {
  ...
  const { order, setOrder, isFetching, refetch } = props;
